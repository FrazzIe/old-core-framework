...
local pfuelcount = 0
Citizen.CreateThread(function()
    for _, item in pairs(gasblips) do
            ...
            if(fuelPumpNear(veh)) then
                if veh and GetPedInVehicleSeat(veh, -1) == playerPed then
                    ...
                    if fuelcount < 100000 then
	                    if not hold then
	                    	drawTxt("Hold ~g~E~w~ to refuel!",0,1,0.5,0.8,0.6,255,255,255,255)
	                    end
	                    ...
	                    if hold then
	                    	drawTxt("~g~Refueling...",0,1,0.5,0.8,0.6,255,255,255,255)
	                    end
	                else
	                	drawTxt("~r~Tank is full!",0,1,0.5,0.8,0.6,255,255,255,255)
	                end
                end
            end
        end
