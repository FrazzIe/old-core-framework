...
local currentmodel = 's_m_y_cop_01'
local policeclothing_emplacement = {
    {name="Police Clothing", id=365, x=454.365753173828, y=-989.097778320313, z=30.6895790100098}, -- Mission row
    {name="Police Clothing", id=365, x=-448.16, y=6008.04, z=31.72}, -- Paleto Bay
    {name="Police Clothing", id=365, x=1848.49, y=3690.13, z=34.27}, -- Sandy Shores
		            ...
		            if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 15.0)then
		                ...
		                elseif(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) > 2.0)then
		                    ...
		                    if cmenu.show ~= 0 then
								...
								TriggerServerEvent("police:saveclothing", currentmodel, drawables, textures)	
							end
		                    cmenu.show = 0
							cmenu.row = 0
							cmenu.field = 0
...
function ClothShop()
	...
	elseif cmenu.show == 4 then
		local drawstr = string.format("ID: %d~n~Name: %s",model_id, policeskins[model_id])
		drawClothingTxt(0.28, 0.8, 0, 0, 0.50,drawstr,255,255,255,255,6)
		DrawRect(0.12,0.388,0.22,0.035,40, 40, 40, 190)
		DrawRect(0.12,0.423,0.22,0.035,40, 40, 40, 190)
		...
		if cmenu.row == 2 then DrawRect(0.628,0.427,0.18,0.035,127, 140, 140, 240) else DrawRect(0.628,0.427,0.18,0.035,40, 40, 40, 190) end
		if cmenu.row == 3 then DrawRect(0.628,0.464,0.18,0.035,127, 140, 140, 240) else DrawRect(0.628,0.464,0.18,0.035,40, 40, 40, 190) end
		local draw_str = string.format("Slot: < %d / "..#policeskins.." >", model_id)
		drawClothingTxt(0.628,0.387,0.175,0.035, 0.50,draw_str,255,255,255,255,6)
		draw_str = string.format("%s", policeskins[model_id])
		...
		if IsControlJustPressed(1, 189) or IsDisabledControlJustPressed(1, 189) then -- left
			if cmenu.row == 1 then
				if model_id > 1 then
					...
					model_id = #policeskins
				end
			end
		end
		if IsControlJustPressed(1, 190) or IsDisabledControlJustPressed(1, 190) then -- right
			if cmenu.row == 1 then
				if model_id < #policeskins then
					model_id=model_id+1
				else
					model_id = 1
				end
		...
		if IsControlJustPressed(1, 201) or IsDisabledControlJustPressed(1, 201) then -- Enter
			if cmenu.row == 1 or cmenu.row == 2 then
				ChangeToSkin(policeskins[model_id])
			elseif cmenu.row == 3 then
				if text_in == 0 then
					text_in = 1
...
RegisterNetEvent("police:ped")
AddEventHandler("police:ped", function(data)
...
RegisterNetEvent("police:freemode")
AddEventHandler("police:freemode", function(data)
    SetPedComponentVariation(GetPlayerPed(-1), 0, tonumber(data.d1), tonumber(data.t1), 0)
    SetPedComponentVariation(GetPlayerPed(-1), 1, tonumber(data.d2), tonumber(data.t2), 0)
    SetPedComponentVariation(GetPlayerPed(-1), 2, tonumber(data.d3), tonumber(data.t3), 1)
    SetPedComponentVariation(GetPlayerPed(-1), 3, tonumber(data.d4), tonumber(data.t4), 0)
	...
	local hashskin2 = GetHashKey("mp_f_freemode_01")
	if GetEntityModel(GetPlayerPed(-1)) == hashskin then
		...
		SetPedComponentVariation(GetPlayerPed(-1), 6, 24, 0, 2)   --Shooes
		SetPedComponentVariation(GetPlayerPed(-1), 10, 8, 0, 2)   --rank
	elseif GetEntityModel(GetPlayerPed(-1)) == hashskin2 then
		SetPedPropIndex(GetPlayerPed(-1), 1, 11, 3, 2)           --Sunglasses
		SetPedPropIndex(GetPlayerPed(-1), 2, 0, 0, 2)            --Bluetoothn earphone
		SetPedComponentVariation(GetPlayerPed(-1), 3, 14, 0, 2)  --Non buggy tshirt
		SetPedComponentVariation(GetPlayerPed(-1), 11, 48, 0, 2) --Shirt
...
RegisterNetEvent("police:nomodel")
AddEventHandler("police:nomodel", function(data)
...
RegisterNetEvent("police:changemodel")
AddEventHandler("police:changemodel",function(data)
    ChangeToSkin(data.m)
    currentmodel = data.m
    TriggerEvent("police:ped", data)
end)

RegisterNetEvent("police:changempmodel")
AddEventHandler("police:changempmodel",function(data)
    ChangeToSkin(data.m)
    currentmodel = data.m
    TriggerEvent("police:freemode", data)
end)
